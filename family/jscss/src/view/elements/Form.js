export function Form({datum, rel_datum, store, rel_type, card_edit, postSubmit, card_display, edit: {el, open, close}}) {
        ...
        <form>
          <div>
            <div style="text-align: left">
              <span style="display: ${datum.to_add || !!rel_datum ? 'none' : null}; float: right; cursor: pointer" class="red-text delete">Удалить</span>
            </div>
            <div>
              <label><input type="radio" name="gender" value="M" ${datum.data.gender === 'M' ? 'checked' : ''}><span>Мужчина</span></label><br>
              <label><input type="radio" name="gender" value="F" ${datum.data.gender === 'F' ? 'checked' : ''}><span>Женщина</span></label><br>
            </div>
          </div>
  ...
  function otherParentSelect() {
    ...
    return (`
      <div>
        <label>Выбрать другой вариант:</label>
        <select name="other_parent" style="display: block">
          ...
          <option value="${'_new'}">Добавить супруг(-а/-у)</option>
        </select>
      </div>
    `)
  }
